...
constexpr size_t kMemoryRegionSize = 8192;
...
int ServerMain(int argc, char **argv) {
    ...
    auto msg_buf = Buffer::Alloc(kMessageBufferSize, kBufAlign);
    auto data_buf = Buffer::Alloc(kMemoryRegionSize, kBufAlign);

    struct fid_mr *msg_mr = net.RegisterMemory(msg_buf);
...
int ClientMain(int argc, char **argv) {
    CHECK(argc >= 3);
    Address server_addr = Address::Parse(argv[1]);
